...
class StageUP(StageC):
    ...
    def _down_encode(self, x, r_embed, clip, cnet=None, require_q=False, lr_guide=None, r_emb_lite=None, guide_weight=1.0, guide_mode_weighted=False, pag_patch_flag=False):
    ...
    def _up_decode(self, level_outputs, r_embed, clip, cnet=None, require_ff=False, agg_f=None, r_emb_lite=None, guide_weight=1.0, guide_mode_weighted=False, pag_patch_flag=False, sag_func=None):
    ...
    def forward(self, x, r, clip_text, clip_text_pooled, clip_img, control=None, **kwargs):
        ...
        patch = kwargs.get("transformer_options", {}).get("patches_replace", {}).get("ultracascade", {}).get("main", None)
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            if patch is not None and patch.x_lr is not None:
                extra_options = {
                    ...
                    'clip_img': clip_img,
                    'cnet': cnet,
                    'sigmas': kwargs.get("transformer_options", {}).get('sigmas', torch.tensor([999999999.9])),
                    'pag_patch_flag': pag_patch_flag,
                    'sag_func': sag_func,
                    'sigmas': sigmas,
                }
